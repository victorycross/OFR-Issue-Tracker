"""
OFR Issue Deck PPTX generation for the Azure Function.

Provides:
- generate_pptx_bytes(data, template_path) — generates the complete PPTX deck in memory

Refactored from tools/generate_issue_deck.py into library form (no CLI, no file I/O).
All slide builders are reused verbatim.
"""
...
def build_cover(prs, data):
    ...
    subtitle_text = data.get("report_subtitle", "Weekly Issue Report")
...
def generate_pptx_bytes(data, template_path):
    """Generate the complete OFR Issue Deck PPTX and return it as bytes.

    Args:
        data: dict with keys: generated_at, report_title, report_subtitle, issues
        template_path: Path to the OFR_Issue_Deck_Template.pptx file
    ...
    if not issues:
        logger.warning("No issues found in input data. Generating minimal deck.")

    # Compute stats
    stats = compute_stats(issues)

    logger.info(f"Generating deck: {len(issues)} issues, {stats['total_open']} open, "
                f"{stats['stale']} stale, {stats['high']} high priority")
    ...
    logger.info(f"Deck generated: {slide_count} slides, {len(pptx_bytes):,} bytes")
