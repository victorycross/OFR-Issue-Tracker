...
|------|----|---------|---------|
| OFR Daily Staleness Calculator | `[AUTO-GENERATED-FLOW-ID-STALENESS]` | Recurrence (daily 6 AM) | Calculates DaysSinceUpdate and sets StalenessFlag for all open issues |
| OFR Intake Promotion | `[AUTO-GENERATED-FLOW-ID-PROMOTION]` | Power Apps V2 trigger | Promotes intake item → creates OFR_Issues entry + UpdateHistory audit trail → marks intake as Promoted → returns new ItemID |
| OFR Generate Issue Deck *(Phase 2)* | `[AUTO-GENERATED-FLOW-ID-DECKGEN]` | Power Apps V2 trigger | Calls Azure Function to generate PPTX deck → returns filename + URL to Power Apps |

### Azure Function (Backend Layer — Phase 2, Optional)

| Property | Value |
|----------|-------|
| Function App | `[AZURE-FUNCTION-APP]` (Azure Functions, Python 3.11, Consumption plan) |
| Endpoint | `POST /api/generate-deck` |
| Auth | Function-level key |
| App Registration | `Bright Path Risk Tracker` (Client ID: `[APP-REGISTRATION-ID]`) |
